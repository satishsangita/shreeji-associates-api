...
import { AppAuthProvider, useAppAuth } from "@/lib/app-auth-context";
import {
 ...
 * Must be inside AppAuthProvider to access useAppAuth.
 */
function AuthNavigator({ children }: { children: React.ReactNode }) {
  const { token, user, loading } = useAppAuth();
  ...
  useEffect(() => {
    ...
    if (!token || !user) {
      ...
      // Logged in and approved — go to main app
      if (inAuthGroup) {
        router.replace("/(tabs)" as any);
      }
    }
    // pending/rejected users stay on auth screen or see blocking UI
